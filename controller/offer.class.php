...
class offer {
	...
	/*początek test dodania daty promowania ogłoszenia*/
	...
	public static function offer($data){
		...
		$sth = $db->prepare('INSERT INTO `'._DB_PREFIX_.'ad`(`user_id`, `name`, `promotion_type`, `date_start`, `days`, `date_end`, `state_id`, `state2_id` ) VALUES (:user_id,:name,:type_id,NOW(),:days,:date_end,:state_id,:state2_id)');
		$sth->bindValue(':user_id', $user->getId(), PDO::PARAM_INT);
		$sth->bindValue(':name', $data['name'], PDO::PARAM_STR);
		$sth->bindValue(':type_id', $data['type_id'], PDO::PARAM_INT);
		$sth->bindValue(':days', $data['days'], PDO::PARAM_INT);
...
//see activateCode
	...
	/*tutaj dodałem drógą datę promowania*/
	public static function enablePromote(int $id,$date=false){
		...
		if($date){
			...
			$promoted_date_end = date('Y-m-d', strtotime($offer['promoted_date_end']. ' + '.$settings['promote_days02'].' days'));
		}else{
			$promoted_date_end = date('Y-m-d', strtotime(date("Y-m-d"). ' + '.$settings['promote_days02'].' days'));
		}
		if($date_finish < $promoted_date_end){
			$date_finish = $promoted_date_end;
		}
		$sth = $db->prepare('UPDATE '._DB_PREFIX_.'offer SET promoted=1, promoted_date_start=NOW(), promoted_date_end=:promoted_date_end, date_finish=:date_finish WHERE id=:id LIMIT 1');
